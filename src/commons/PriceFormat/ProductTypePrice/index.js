...
const SimpleProductTypePrice = ({
    ...
    if (regularPrice.value === finalPrice.value) {
        return (
            <Typography variant={priceLabelVariant} className={cx('price-text', 'text-neutral-400 !text-2xl font-bold', 'price-case-8', textClassName)}>
                {formatPrice(finalPrice.value + otherPrice, finalPrice.currency, currencyCache)}
            </Typography>
        );
    }
    ...
    return (
        <div className={cx('price-case-9 price-text-discount', 'inline-flex', 'flex-col')}>
            {/* case 9 */}
            <Typography variant={priceLabelVariant} className={cx('price-text', 'text-neutral-400 !text-2xl font-bold', textClassName)}>
                {validSpecial
                    ? formatPrice(finalPrice.value + otherPrice, finalPrice.currency, currencyCache)
                    : formatPrice(regularPrice.value + otherPrice, regularPrice.currency, currencyCache)}
            </Typography>
            {validSpecial && (
                <Typography variant={priceLabelSubVariant} className={cx('price-text !text-base', textClassName)}>
                    <strike>{formatPrice(regularPrice.value + otherPrice, regularPrice.currency, currencyCache)}</strike>
                </Typography>
            )}
        </div>
